  ...
  return (
    <section id="services" className="py-20 relative">
      <div className="container">
        <SectionHeading eyebrow="What we do" title="Services" paragraph="Practical automation delivered with clean engineering." />
          ...
          {siteContent.solutions.map((s, i) => (
            <AnimatedCard key={s.name} delay={i * 0.06} className="svc-card">
              ...
              <ul className="mt-4 space-y-2 text-neutral-700">
                {s.bullets.map((x, k) => <li key={k} className="flex gap-2"><span>•</span><span>{x}</span></li>)}
              </ul>
            </AnimatedCard>
          ))}
        </div>
